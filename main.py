...
專案名稱：TOEIC 單字庫處理程式
檔案名稱：main.py
檔案說明：主程式進入點，串接下載、資料清理轉換與 CSV 匯出管線
"""
...
def run_pipeline(output_csv: str = "toeic_vocabulary.csv", cache_json: str = "toeic_raw.json") -> None:
    """
    執行完整的 TOEIC 單字庫下載、清洗轉換與 CSV 匯出流程

    參數：
        output_csv (str): 目標輸出的 CSV 檔案名稱
        cache_json (str): 原始 JSON 暫存檔案名稱
    """
    print("=== 開始執行 TOEIC 單字集下載與清洗轉換 ===")
    ...
    print(f"過濾並清洗後單字筆數 (僅保留單一單字): {len(transformed_items)} 筆")

    # 3. 匯出為 CSV
    CsvExporter.export_to_csv(transformed_items, output_csv)
